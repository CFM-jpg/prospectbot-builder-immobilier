...
// Bouton "Prospecter" : sauvegarde vendeur + crée prospect CRM + envoie email notif
...
async function sendBrevoEmail({ to, subject, html }) {
  ...
  if (!BREVO_API_KEY) {
    console.warn('[Prospecter] BREVO_API_KEY non configurée');
    return;
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── 2. Créer/mettre à jour dans prospects (CRM) ───────────────────────────
    ...
    const { error: prospError } = await supabase
      ...
      .upsert({
        ...
        notes: [
          ...
          vendeur.date || vendeur.date_mutation
            ? `Transaction DVF : ${(vendeur.date || vendeur.date_mutation)?.substring(0, 10)}`
            : null,
        ].filter(Boolean).join(' | '),
    ...
    if (prospError) {
      console.warn('[Prospecter] Prospects upsert warning:', prospError.message);
    }

    // ── 3. Email de notification via Brevo ────────────────────────────────────
        ...
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 24px;">
          ...
          <div style="border: 1px solid #e5e7eb; border-top: none; padding: 24px; border-radius: 0 0 8px 8px;">
            <table style="width: 100%; border-collapse: collapse;">
              ...
              <tr><td style="padding: 8px 0; color: #6b7280;">Prix achat</td><td>${vendeur.prix ? `${vendeur.prix.toLocaleString('fr-FR')} €` : '—'}</td></tr>
            ...
            ${vendeur.latitude && vendeur.longitude ? `
            <div style="margin-top: 16px;">
              <a href="https://www.google.com/maps?q=${vendeur.latitude},${vendeur.longitude}"
            ...
            <p style="margin-top: 24px; color: #9ca3af; font-size: 12px;">
              Ce vendeur a été ajouté à votre pipeline CRM NestLead avec le statut "À contacter".
            </p>
          </div>
        </div>`;
    ...
    return res.status(200).json({
      success: true,
      message: 'Vendeur sauvegardé, ajouté au CRM et email envoyé.',
    });
